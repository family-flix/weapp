...
let newProxy = <T extends object>(target: T, handler: ProxyHandler<T>) => new Proxy(target, handler);
...
type ProxyCache<T extends object> = WeakMap<object, readonly [ProxyHandler<T>, ProxyHandlerState<T>]>;
type TargetCache<T extends object> = WeakMap<object, readonly [target: T, copiedTarget?: T]>;
...
export const createProxy = <T>(
  ...
  let targetAndCopied = targetCache && (targetCache as TargetCache<typeof obj>).get(obj);
  if (!targetAndCopied) {
  ...
  let handlerAndState = proxyCache && (proxyCache as ProxyCache<typeof target>).get(target);
...
export const isChanged = (
  ...
  try {
    ...
    for (const key of used[KEYS_PROPERTY] || []) {
      changed = isChanged((prevObj as any)[key], (nextObj as any)[key], affected, cache, isEqual);
      if (changed) return changed;
    }
 ...
 */
export const affectedToPathList = (obj: unknown, affected: WeakMap<object, unknown>, onlyWithValues?: boolean) => {
