...
history.push = (name: PageKeys, query: Record<string, string> = {}, options: Partial<{ ignore: boolean }> = {}) => {
  ...
  const url = [path, query ? query_stringify(query) : ""].filter(Boolean).join("?");
  wx.navigateTo({
    url,
  });
  return null;
};
history.replace = (name: PageKeys, query: Record<string, string> = {}, options: Partial<{ ignore: boolean }> = {}) => {
  const path = pages[name];
  if (!path) {
    app.tip({
      text: ["异常操作"],
