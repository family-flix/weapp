...
export function validateMemberToken(v: { token: string }) {
  return request.post<{ id: string; token: string }>("/api/validate", {
    token: v.token,
  });
}

export function validateMemberCodeOfWechat(v: { code: string }) {
  console.log("validateMemberCodeOfWechat", v);
  return request.get<{ id: string; token: string }>("/api/validate/wechat", { code: v.code });
}
